...
def get_team_info_from_db(team_name):
    try:
        ...
        # Query to get team and coach information
        team_sql = """
          ...
          WHERE t.team_name = %s
        """
        cursor.execute(team_sql, (team_name,))
        team_data = cursor.fetchone()

        if not team_data:
            return "Team not found."

        # Query to get players in the team
        player_sql = """
          ...
          WHERE t.team_name = %s
        """
        cursor.execute(player_sql, (team_name,))
        player_data = cursor.fetchall()

        team_info = f"Team Name: {team_data[0]}\n"
        team_info += f"Team Coach: {team_data[1]}\n\n"

        team_info += "Players:\n"
        for player in player_data:
            team_info += f"- {player[0]} {player[1]}\n"
